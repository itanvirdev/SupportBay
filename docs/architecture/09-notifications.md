...
First-response SLA breach delivery listens to `TicketSlaBreached`. It queues `ticket_sla_breached:agent` only for the ticket's currently assigned WordPress user when that user still has `sbay_view_tickets` and the matching preference and template are active. The context includes the SLA target, breach time, and calendar minutes overdue. Unassigned tickets and unauthorized users are silent. Because the detector persists each ticket/metric breach before dispatch, repeated worker runs do not create duplicate alerts.
