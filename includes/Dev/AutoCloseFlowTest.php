...
final class AutoCloseFlowTest extends FlowTest{
  ...
  protected static function execute(...$services):void{/** @var TicketService $tickets */[$tickets,$repository,$worker,$settings]=$services;$previous=get_option('sbay_auto_close_settings',[]);$id=$tickets->create(['customer_id'=>1,'department_id'=>1,'subject'=>'Auto lifecycle flow test']);$old=current_datetime()->modify('-2 days')->format('Y-m-d H:i:s');
