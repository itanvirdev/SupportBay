...
final class CategoryRepository extends Repository {
  ...
  public function create(array $data):int{return $this->insert(['name'=>$data['name'],'slug'=>$data['slug'],'description'=>$data['description'],'department_id'=>$data['department_id'],'status'=>$data['status'],'color'=>$data['color'],'sort_order'=>$data['sort_order'],'created_at'=>$this->now(),'updated_at'=>$this->now()],['%s','%s','%s','%d','%s','%s','%d','%s','%s']);}
  ...
  protected function hydrate(array $row):Category{return new Category(id:(int)$row['id'],name:(string)$row['name'],slug:(string)$row['slug'],description:$row['description']!==null?(string)$row['description']:null,departmentId:isset($row['department_id'])?(int)$row['department_id']:null,status:CategoryStatus::from($row['status']),color:$row['color']!==null?(string)$row['color']:null,sortOrder:(int)$row['sort_order'],createdAt:(string)$row['created_at'],updatedAt:(string)$row['updated_at']);}
}
