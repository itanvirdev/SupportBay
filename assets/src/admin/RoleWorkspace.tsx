...
export function RoleWorkspace(){
  ...
  const load=useCallback(async()=>{setLoading(true);setError(null);try{const response=await adminGet<SupportRole[]>('roles');setRoles(response.data);setGroups((response.meta.capability_groups??{}) as CapabilityGroups);setRequired((response.meta.required_capabilities??[]) as string[]);}catch(reason){setError(reason instanceof Error?reason.message:'Roles could not be loaded.');}finally{setLoading(false);}},[]);
  ...
  return <section className="sbay-role-settings">
    ...
    <div className="sbay-role-table"><div className="is-header"><span>Name</span><span>Users</span><span>Status</span><span>Action</span></div>{loading?<Preloader label="Loading user roles…" />:roles.map(role=><div key={role.slug}><span><strong>{role.name}</strong>{role.built_in?<small>Built-in</small>:null}</span><span>{role.user_count}</span><span className={`is-${role.status}`}>{role.status}</span><span className="sbay-role-actions"><button type="button" onClick={()=>setEditing({...role,capabilities:[...role.capabilities]})}>{role.editable?'Edit':'View'}</button>{role.editable?<button type="button" className="is-danger" disabled={saving||role.user_count>0} title={role.user_count?'Remove assigned users before deleting this role.':'Delete role'} onClick={()=>void remove(role)}>Delete</button>:null}</span></div>)}</div>
