...
export function DepartmentWorkspace(){
  ...
  return <section className="sbay-department-settings">
    ...
    <div className="sbay-department-table"><div className="is-header"><span>Name</span><span>Priority</span><span>Order</span><span>Status</span><span>Action</span></div>{loading?<p>Loading…</p>:items.map(item=><div key={item.id}><span><strong>{item.name}</strong>{item.slug==='support'?<small>Default</small>:null}</span><span>{item.default_priority}</span><span>{item.sort_order}</span><span className={`is-${item.status}`}>{item.status}</span><span className="sbay-department-actions"><button type="button" onClick={()=>setSelected({...item})}>Edit</button>{item.slug!=='support'?<button type="button" className="is-danger" disabled={saving} onClick={()=>void remove(item)}>Delete</button>:null}</span></div>)}</div>
