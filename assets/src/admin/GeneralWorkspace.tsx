  ...
  return <section className="sbay-general-settings">
    ...
    <form className="sbay-general-card" onSubmit={save}>
      ...
      {activeTab==='main'?<div className="sbay-general-main">
        ...
        <label className="sbay-general-select"><span>Client User Default Role</span><select disabled={saving} value={draft.client_user_default_role} onChange={event=>update({client_user_default_role:event.target.value})} aria-describedby="sbay-client-role-help">{settings.role_options.map(role=><option value={role.slug} key={role.slug}>{role.name}</option>)}</select><small id="sbay-client-role-help">New SupportBay registrations receive this WordPress role. Choose a staff role only when public registration is intentionally restricted.</small></label>
