...
type SettingsSection = 'general'|'roles'|'departments'|'categories'|'tags'|'custom-fields'|'sla'|'saved-replies'|'notifications'|'logs'|'integrations';

export function SettingsWorkspace(){
  const config=getAdminConfig();
  const [section,setSection]=useState<SettingsSection>('general');
  return <section className="sbay-settings-workspace"><nav aria-label="Settings sections">
    <button type="button" className={section==='general'?'is-active':''} onClick={()=>setSection('general')}>General</button><span>Security</span>
  ...
  </nav><div>{section==='general'?<GeneralWorkspace/>:section==='roles'?<RoleWorkspace/>:section==='departments'?<DepartmentWorkspace/>:section==='categories'?<CategoryWorkspace/>:section==='tags'?<TagWorkspace/>:section==='custom-fields'?<CustomFieldWorkspace/>:section==='sla'?<TicketSlaWorkspace/>:section==='saved-replies'?<SavedReplyWorkspace/>:section==='notifications'?<NotificationTemplateWorkspace/>:section==='logs'?<NotificationLogWorkspace/>:<ProviderWorkspace/>}</div></section>;
}
