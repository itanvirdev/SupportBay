    ...
    <section className={`sbay-ticket-workspace sbay-ticket-workspace--${mode}`}>
      ...
      <div className="sbay-ticket-filters">
        ...
        <div className="sbay-ticket-filter-row">
          ...
          {mode==='staff'&&query.assignment!=='mine'?<select aria-label="Agent" value={query.agentId} onChange={event=>update({agentId:event.target.value,assignment:''})}><option value="">All Agents</option>{options?.agents.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select>:null}
