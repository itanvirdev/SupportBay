...
export function TicketWorkspace({ mode, load, openTicket, createTicket, options, bulk, openCustomers, openVerifications, autoRefresh, needReplyFilterEnabled = true, statusLabels = {} }: TicketWorkspaceProps) {
  ...
  const showDepartments = mode === 'staff'
    ? (options?.departments.length ?? 0) > 1
    : Boolean(result?.showDepartments);
    ...
    <section className={`sbay-ticket-workspace sbay-ticket-workspace--${mode}`}>
      ...
      <div className="sbay-ticket-filters">
        ...
        <div className="sbay-ticket-filter-row">
          {showDepartments?<select aria-label="Department" value={query.departmentId} onChange={event=>update({departmentId:event.target.value,categoryId:'',customFieldId:'',customFieldValue:''})}><option value="">All Departments</option>{options?.departments.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select>:null}
          {showCategories?<select aria-label="Category" value={query.categoryId} onChange={event=>update({categoryId:event.target.value})}><option value="">All Categories</option><option value="uncategorized">Uncategorized</option>{filterCategories.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select>:null}
          {showTags?<select aria-label="Tag" value={query.tagId} onChange={event=>update({tagId:event.target.value})}><option value="">All Tags</option>{options?.tags.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select>:null}
