...
interface Props {mode:WorkspaceMode;query:TicketQueryParams;update:(changes:Partial<TicketQueryParams>)=>void;refresh:()=>void;createTicket?:()=>void;openCustomers?:()=>void;openVerifications?:()=>void}

export function TicketQueueTabs({mode,query,update,refresh,createTicket,openCustomers,openVerifications}:Props) {
  return <div className="sbay-ticket-tabs" role="tablist" aria-label="Ticket queues">
    ...
    <div className="sbay-ticket-tabs__actions"><button aria-label="Refresh tickets" onClick={refresh}>↻</button>{openCustomers?<button onClick={openCustomers}>Customers</button>:null}{openVerifications?<button onClick={openVerifications}>Verifications</button>:null}{createTicket?<button className="is-primary" onClick={createTicket}>＋ Add Ticket</button>:null}</div>
  </div>;
}
